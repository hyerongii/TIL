...
//     - [x] CSS 스타일 파일 연결
//     - [x] class 속성으로 스타일 설정
//     - [x] style 속성으로 스타일 설정 (HTML에서는 string, JSX에서는 object)
// --------------------------------------------------------------------------

import { createRoot } from 'react-dom/client'

// 절대 경로(등록된 @ 별칭)를 사용해 CSS 스타일 파일 불러오기
...
const infoListStyles = {
  border: '8px double #d46935',
  backgroundColor: '#fee3d5',
}
...
const infoSectionElement = (
  <section
    id="info"
    // class는 예약어이므로 className으로 작성
    className="info-section"
    // 속성에 데이터 할당
    aria-label={sectionLabel}
    // aria-*, data-* 속성은 케밥케이스 유형으로 작성
    data-react-id="r-smkd-1ds"
  >
    <div className="section-inner">
      <h3 className="section-header">
        ...
        <span className="text-orange-900">
          국내 도서 분야 앱 {rank}위
          <br />
          {/* 함수 호출 결과 값인 데이터 끼워넣어 연결 */}
          무제한 혜택이 {convertPriceWithComma(price)}원
        </span>
      </h3>
      <ul className="info-content">
        <li className="info-list" style={infoListStyles}>
          ...
          <div className="info-content-box">
            온 가족이 보는{' '}
            <em className="text-orange-700 font-normal">‘패밀리 라운지’</em>{' '}
            부터 <br />
            판타지/로맨스 장르 <span className="text-orange-500">
              ‘스토리’
            </span>{' '}
            까지
          </div>
